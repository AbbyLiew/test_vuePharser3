...
export default class GreenScene extends Scene {
  constructor() {
    super({ key: "GreenScene" });
    this.acc = 0;
    this.video_green = null;
  ...
  preload() {
    for (let i = 23; i < 84; i++) {
      ...
      this.load.image(
        "GreenSET_" + i,
        "/videos/sequance_green/" + assetname + ".jpg"
      );
    }
    this.load.video(
      "purple",
    ...
    this.load.image("hand_green", "/icon/lightgreen.png");
  ...
  vidioFadeIn() {
    this.tweens.add({
      targets: this.video_green,
      alpha: 1,
      duration: 1000,
      ease: "Linear",
      delay: 500,
      onComplete: () => {
        this.video_green.play(false, 0, 10);

        this.video_green.on("complete", () => {
  ...
  create() {
    this.addSprite();
    this.hand = this.add.image(0, 0, "hand_green");
    ...
    this.video_green = this.add.video(0, 0, "purple");
    this.video_green.setAlpha(1);
    this.video_green.stop();

    this.video_green.setDisplaySize(
      this.sys.game.config.height,
      this.sys.game.config.height
    );
    this.video_green.setPosition(
      this.sys.game.config.width / 2,
      this.sys.game.config.height / 2
    );

    this.video_green.setDepth(1);
    this.video_green.setAlpha(0);
    ...
    this.sprite.on("dragend", (e) => {
      ...
      if (e.downX >= e.x) {
        if (this.playedCard1 === false) {
          this.sprite.anims.play("play_card1_green");
          ...
          this.sprite.anims.play("play_card2_green");
        ...
        if (this.playedCard2 === true) {
          this.sprite.anims.play("play_card2_reverse_green");
          this.playedCard2 = false;
          this.hand_white2.destroy();

          this.hand.setAlpha(0);
        } else if (this.playedCard1 === true) {
          this.sprite.anims.play("play_card1_reverse_green");
          this.playedCard1 = false;
          this.hand_white.destroy();
